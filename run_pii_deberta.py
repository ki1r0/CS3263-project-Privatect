...
class DebertaDetector:
    def __init__(self, model_path=None):
        """Initialize DeBERTa-based PII detector"""
        # Always default to local DeBERTa model unless explicitly overridden
        requested_path = model_path or "pii_deberta_base_v1/final"
        self.model_path = self._resolve_local_model_path(requested_path)
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
        print(f"🤖 Loading DeBERTa PII Detection Model...")
    ...
    def _resolve_local_model_path(self, req_path: str) -> str:
        """Find a usable local model directory for DeBERTa fine-tuned weights.

        Tries common variants to avoid configuration hassles. Returns a path
        that exists and contains a HuggingFace model (config.json present),
        or raises a clear error with guidance.
        """
        ...
        add(os.path.normpath(req_path))
        # As provided relative to this file's directory
        add(os.path.normpath(os.path.join(here, req_path)))

        # If pointing at .../final, also try its parent and vice versa
        norm = os.path.normpath(req_path)
        if norm.lower().endswith(os.path.normpath("final")):
            add(os.path.normpath(os.path.join(norm, os.pardir)))
            add(os.path.normpath(os.path.join(here, norm, os.pardir)))
        else:
            add(os.path.normpath(os.path.join(norm, "final")))
            add(os.path.normpath(os.path.join(here, norm, "final")))

        # Common alternative layouts
        add(os.path.normpath(os.path.join(cwd, "models", "pii_deberta_base_v1", "final")))
        add(os.path.normpath(os.path.join(cwd, "models", "pii_deberta_base_v1")))
        add(os.path.normpath(os.path.join(cwd, "pii_deberta_base_v1")))

        # Validate candidates: must be a dir and contain config.json
        checked = []
        ...
        msg = [
            "Could not find local DeBERTa model directory.",
            "Expected a folder containing HuggingFace files (e.g., config.json, tokenizer.json, model.safetensors)",
            "Place your fine-tuned model here:",
            f"  - {os.path.join(cwd, 'pii_deberta_base_v1', 'final')} (preferred)",
            f"  - or {os.path.join(cwd, 'pii_deberta_base_v1')} (with config.json inside)",
            "Searched the following locations:",
        ]
        msg += [f"  - {p}" for p in checked]
        raise FileNotFoundError("\n".join(msg))

    def predict_pii(self, text, threshold=0.5):
        """Predict PII labels using DeBERTa model"""
...
def create_pii_gui():
    ...
    class PIIDetectorGUI:
        def __init__(self):
            self.root = ctk.CTk()
            self.root.title("🔍 PII Detection Tool - DeBERTa")
        ...
        def create_ui(self):
            ...
            self.status_label = ctk.CTkLabel(
                self.root,
                text="🔄 Loading DeBERTa PII detection model...",
        ...
        def load_model(self):
            ...
            def load_in_background():
                try:
                    # Load default DeBERTa model
                    self.detector = DebertaDetector()
                    self.root.after(0, lambda: self.status_label.configure(
                        text="✅ DeBERTa PII detection model loaded successfully!",
...
def main():
    """Main function to run the DeBERTa PII detector"""
    import sys
    if len(sys.argv) > 1 and sys.argv[1] == "--gui":
        ...
        # Run CLI version
        print("="*80)
        print("🤖 DEBERTA PII DETECTION MODEL")
        print("Running DeBERTa base model locally")
