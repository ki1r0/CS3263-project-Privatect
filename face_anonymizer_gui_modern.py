...
# from tkinter_tooltip import ToolTip  # Optional external library (unused)

# Lightweight tooltip helper for hover text
class _ToolTip:
...
# Import PII detection functionality (DeBERTa-based)
from run_pii_deberta import DebertaDetector
...
class ModernMultiFunctionGUI:
    def __init__(self):
        # Create main window
        self.root = ctk.CTk()
        self.root.title("🔒 Privatect - Face & Text Privacy Studio")
    ...
    def create_text_input_tab(self):
        ...
        # Output area (top) with inline toolbar
        output_header = ctk.CTkFrame(text_areas_frame, fg_color="transparent")
        ...
        output_tools = ctk.CTkFrame(output_header, fg_color="transparent")
        output_tools.pack(side="right")
        ...
        _btn_save_out = ctk.CTkButton(output_tools, text="💾", command=self.save_output_file, **small_btn)
        _btn_save_out.pack(side="left", padx=3)
        attach_tooltip(_btn_save_out, "Save Output")

        _btn_copy_out = ctk.CTkButton(output_tools, text="📋", command=self.copy_output_to_clipboard, **small_btn)
        _btn_copy_out.pack(side="left", padx=3)
        attach_tooltip(_btn_copy_out, "Copy Output to Clipboard")

        _btn_clear_out = ctk.CTkButton(output_tools, text="🗑", command=self.clear_text_output, **small_btn)
        _btn_clear_out.pack(side="left", padx=3)
        attach_tooltip(_btn_clear_out, "Clear Output")
        ...
        # Input area (bottom) with inline toolbar
        input_header = ctk.CTkFrame(text_areas_frame, fg_color="transparent")
        ...
        input_tools = ctk.CTkFrame(input_header, fg_color="transparent")
        input_tools.pack(side="right")
        
        # Process (direct masking) first
        _btn_process = ctk.CTkButton(input_tools, text="⏎", command=self.direct_mask_to_output, **small_btn)
        _btn_process.pack(side="left", padx=3)
        attach_tooltip(_btn_process, "Process (Direct Masking)")

        # Summary second
        _btn_mask = ctk.CTkButton(input_tools, text="🪄", command=self.detect_pii_text, **small_btn)
        _btn_mask.pack(side="left", padx=3)
        attach_tooltip(_btn_mask, "PII Summary")

        _btn_open = ctk.CTkButton(input_tools, text="📁", command=self.open_text_file, **small_btn)
        _btn_open.pack(side="left", padx=3)
        attach_tooltip(_btn_open, "Open Text File")

        _btn_clear_in = ctk.CTkButton(input_tools, text="🗑", command=self.clear_text_input, **small_btn)
        _btn_clear_in.pack(side="left", padx=3)
        attach_tooltip(_btn_clear_in, "Clear Input")
        ...
        # Controls below are now integrated into toolbars; keep area uncluttered
        # (rows removed as per request)
        ...
        # Shortcut: Ctrl+Enter to run PII Summary
        self.text_input_area.bind('<Control-Return>', self.on_input_ctrl_enter)
        self.text_output_area.bind('<KeyRelease>', self.update_text_stats)
        self.text_output_area.bind('<Button-1>', self.update_text_stats)

    def on_input_ctrl_enter(self, event=None):
        try:
            self.detect_pii_text()
        finally:
            return "break"

    def copy_output_to_clipboard(self):
        try:
            text = self.text_output_area.get(1.0, tk.END)
    ...
    def create_video_tab(self):
        """Create a simple video face masking tab (blur/pixelate)."""
        main = ctk.CTkScrollableFrame(self.video_tab, width=860, height=540)
        main.pack(fill="both", expand=True, padx=10, pady=10)

        ctk.CTkLabel(main, text="🎬 Video Face Masking", font=ctk.CTkFont(size=20, weight="bold")).pack(pady=(10, 6))
        ctk.CTkLabel(main, text="Efficient frame-by-frame anonymization using YOLO face detection", font=ctk.CTkFont(size=13), text_color=("gray60","gray40")).pack()

        # Paths
        self.video_input_path = tk.StringVar()
        self.video_output_path = tk.StringVar()
        self.video_mode = tk.StringVar(value="blur")
        self.video_threshold = tk.DoubleVar(value=0.5)

        paths = ctk.CTkFrame(main)
        paths.pack(fill="x", padx=10, pady=10)

        # Input
        row_in = ctk.CTkFrame(paths, fg_color="transparent")
        row_in.pack(fill="x", pady=4)
        ctk.CTkLabel(row_in, text="Input Video:", font=ctk.CTkFont(size=12, weight="bold")).pack(side="left")
        self.video_in_entry = ctk.CTkEntry(row_in, textvariable=self.video_input_path, width=520)
        self.video_in_entry.pack(side="left", padx=8, fill="x", expand=True)
        btn_in = ctk.CTkButton(row_in, text="Browse", command=self.browse_video_input, width=80)
        btn_in.pack(side="left", padx=4)

        # Output
        row_out = ctk.CTkFrame(paths, fg_color="transparent")
        row_out.pack(fill="x", pady=4)
        ctk.CTkLabel(row_out, text="Output Video:", font=ctk.CTkFont(size=12, weight="bold")).pack(side="left")
        self.video_out_entry = ctk.CTkEntry(row_out, textvariable=self.video_output_path, width=520)
        self.video_out_entry.pack(side="left", padx=8, fill="x", expand=True)
        btn_out = ctk.CTkButton(row_out, text="Browse", command=self.browse_video_output, width=80)
        ...
        ctk.CTkLabel(opts, text="Masking Mode:", font=ctk.CTkFont(size=12, weight="bold")).pack(anchor="w")
        modes = ctk.CTkFrame(opts, fg_color="transparent")
        modes.pack(anchor="w", pady=(2,6))
        ctk.CTkRadioButton(modes, text="Blur", variable=self.video_mode, value="blur").pack(side="left", padx=8)
        ctk.CTkRadioButton(modes, text="Pixelate", variable=self.video_mode, value="pixelate").pack(side="left", padx=8)

        thr = ctk.CTkFrame(opts, fg_color="transparent")
        thr.pack(fill="x", pady=(8,2))
        ctk.CTkLabel(thr, text="Detection Threshold:", font=ctk.CTkFont(size=12, weight="bold")).pack(side="left")
        slider = ctk.CTkSlider(thr, from_=0.2, to=0.9, variable=self.video_threshold, width=250)
        slider.pack(side="left", padx=10)
        self.video_thr_label = ctk.CTkLabel(thr, text="0.5")
        self.video_thr_label.pack(side="left")
        self.video_threshold.trace('w', lambda *_: self.video_thr_label.configure(text=f"{self.video_threshold.get():.2f}"))

        # Process button and status
        actions = ctk.CTkFrame(main, fg_color="transparent")
        actions.pack(pady=10)
        self.video_process_btn = ctk.CTkButton(actions, text="Run Masking", command=self.process_video_click, width=160, height=36)
        self.video_process_btn.pack(side="left", padx=6)
        self.video_status = ctk.CTkLabel(actions, text="Idle", text_color=("gray70","gray50"))
        self.video_status.pack(side="left", padx=10)

        self.video_progress = ctk.CTkProgressBar(main, width=640)
        self.video_progress.set(0)
        self.video_progress.pack(pady=(6,12))

    def browse_video_input(self):
        path = filedialog.askopenfilename(title="Select Video", filetypes=[("Video","*.mp4;*.avi;*.mov;*.mkv;*.webm")])
        if path:
    ...
    def process_video_click(self):
        inp = self.video_input_path.get().strip()
        outp = self.video_output_path.get().strip()
        if not inp:
            messagebox.showwarning("Missing Input", "Please select an input video")
            return
        if not outp:
            messagebox.showwarning("Missing Output", "Please select an output path")
            return
        if self.model is None:
            messagebox.showwarning("Model Not Loaded", "Face detection model not loaded yet. Please wait a moment and try again.")
            return
        self.video_process_btn.configure(state="disabled")
        self.video_status.configure(text="Processing...")

        def worker():
            try:
                self._process_video_file(inp, outp, self.video_mode.get(), float(self.video_threshold.get()))
                self.root.after(0, lambda: self.video_status.configure(text="Done ✔"))
            except Exception as e:
                self.root.after(0, lambda: self.video_status.configure(text=f"Error: {e}"))
            finally:
                self.root.after(0, lambda: self.video_process_btn.configure(state="normal"))

        threading.Thread(target=worker, daemon=True).start()

    def _process_video_file(self, in_path, out_path, mode, thr):
        ...
        while True:
            ...
            if not ret:
                break
            faces = detect_faces_yolo(frame, self.model, thr)
            if faces:
                if mode == 'pixelate':
                    frame = anonymize_pixelate(frame, faces, self.blocks.get() if hasattr(self, 'blocks') else 12)
                else:
                    frame = anonymize_blur(frame, faces, self.blur_strength.get() if hasattr(self, 'blur_strength') else 0)
            out.write(frame)
            processed += 1
            if total:
        ...
        def load_model():
            try:
                ...
                def update_error(err=e):
                    self.status_label.configure(text=f"⚠️ Warning: {str(err)}")
    ...
    def load_pii_model(self):
        ...
        def load_model():
            try:
                ...
                self.pii_detector = DebertaDetector()
    ...
    def direct_mask_to_output(self):
        """Detect and mask PII directly into the Output Area."""
        text = self.text_input_area.get(1.0, tk.END).strip()
        if not text:
            messagebox.showwarning("Warning", "Please enter text in the Input Area")
            return
        if self.pii_detector is None:
            messagebox.showerror("Error", "PII detection model not loaded yet. Please wait and try again.")
            return
        try:
            ner_results, pii_found, _ = self.pii_detector.predict_pii(text, self.pii_threshold.get())
            # Build masked text using non-overlapping spans
            spans = [r for r in ner_results if r.get('score', 0) >= self.pii_threshold.get()]
            spans.sort(key=lambda r: (r['start'], -r['end']))
            # Merge overlapping by keeping the longest
            merged = []
            for r in spans:
            ...
            for r in merged:
                s, e = r['start'], r['end']
                # Append original text before the span
                masked_chunks.append(text[last:s])
                # Mask with constant token 'xxxx' and add spaces if adjacent are alphanumerics
                pre_space = " " if s > 0 and text[s-1].isalnum() else ""
                post_space = " " if e < len(text) and text[e:e+1].isalnum() else ""
                masked_chunks.append(pre_space + "xxxx" + post_space)
                last = e
            masked_chunks.append(text[last:])
            masked_text = ''.join(masked_chunks)
            # Fallback masking for emails missed by the model
            email_pattern = re.compile(r"(?i)\b[\w.%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b")
            masked_text = email_pattern.sub("xxxx", masked_text)
            self.text_output_area.delete(1.0, tk.END)
            self.text_output_area.insert(1.0, masked_text)
            self.update_text_stats()
        except Exception as e:
            messagebox.showerror("PII Masking Error", f"Failed to mask text:\n{str(e)}")
